...
void map_save(struct Robot *robot) {
  ...
  fprintf(myfile, "Finish position: %d.%d\n\n", // Two linebreaks for blank line

  robot->map.finish.x, robot->map.finish.y);
  fprintf(myfile, "Map segments:"); // No line break it gets written in for loop
  ...
  for (int i = 0; i < robot->map.size.y; ++i) { // loop rows
    ...
    for (int j = 0; j < robot->map.size.x; ++j) { // loop cols
      ...
      // Put a space after each map segment except for last segment in the row
