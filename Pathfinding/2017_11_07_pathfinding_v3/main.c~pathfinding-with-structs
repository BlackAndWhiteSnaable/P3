...
void path_calculate(char map[][5], int cost[][5], char parent[][5], int startX, int startY, int stopX,  int stopY);
...
int main()
{
  ...
  path_calculate(map, cost, parent, 4, 0, 4, 4);
  return 0;
}
...
char path_evaluate_node(char map[][5], int cost[][5], char parent[][5], int x, int y, int finalX, int finalY)
{
  if (x==finalX && y==finalY) return (char)0xFF;
  ...
  if (map[x][y]&N)
  {
    ...
    if (cost[x][y]+COST_90 < cost[nextX][nextY])
    {
  ...
  if (map[x][y]&NE)
  {
    ...
    printf("\t(%i,%i)\n",nextX,nextY);
    //compares the currently stored cost with the newly calculated
    if (cost[x][y]+COST_45 < cost[nextX][nextY])
    {
...
void path_calculate(char map[][5], int cost[][5], char parent[][5], int startX, int startY, int stopX,  int stopY)
{
  ...
  if (path_evaluate_node(map,cost,parent,startX,startY,stopX,stopY)==(char)0xFF)
  {
