...
This is known as a graph, and is explained more in detail in Section \ref{sec:graph}. \todo{Daniel your Graph section is missing a label}

More blabla.. It is possible to convert the ordinary map made of ASCII characters, to a map that only consists of nodes.
A struct was created with a data structure that could hold all the necessary data for each node.
\todo{insert/refer to node struct}
...
Every time a change in the map is detected, the path finding function is called\todo{insert function name}. This calculates the most efficient path from the robots current location to the finish using the updated map data. 
