...
Our implementation of the path-finding algorithm relies heavily on our program written in C.
Every function explained in this section can also be read in Appendix \ref{app:path}.

Since our map consists of \emph{nodes} with up to eight neighbors,
...
This effectively limits the map size we can handle, but can be expanded later on.
...
To prevent the algorithm from looping infinitely in case of a subtle error,
we increment a counter {\tt deadcount} every iteration
...
and whether its \emph{cost to reach} is bigger than it would be from the current \emph{node}.
...
%
After we looked through all neighbours of a \emph{node},
...
\lstinputlisting
[firstline=127,			%starts reading the file from line 64
firstnumber=127,
lastline=131,			%stops reading the file at line 88
label=lst:node_calc,	%label
caption={Pushing and Popping Nodes in {\tt path.c}}
]{code/path.c}
%
After looking through all \emph{nodes} leading to the finish \emph{node},
