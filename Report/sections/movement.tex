...
Figure \ref{fig:mot_ctrl} represents a schematic of the movement circuit. We have used six Tri-State 
buffers, four active high, and two active low. 
\begin{figure}[htp]
	\centering
...
The four wires coming from the microcontroller, labeled '1', '2', '3' and '4' correspond to the stepping sequence.
They are each connected to two Tri-State buffers corresponding to each pair of motors. One pair of motors consists 
of two motors. The pairs are represented in Figure \ref{fig:motor_pairs_location}.
...
\clearpage
The first two Tri-State buffers, decide whether the pair is needed for moving in 
a specific direction. The input pins, labeled ‘A Enable’ and ‘B Enable’ go to the 
two Tri-State buffers connected to each motor pair. 

Afterwards, each buffer that enables the pair of motors 
is connected to two buffers that decide the direction, one active-high and one active-low. 
‘A Direction' decides the direction for ‘Pair A’, while ‘B Direction’ decides the direction 
for ‘Pair B’.

For example, when moving North-East, only ‘Pair A’ is needed. ‘A Enable’ will have a ‘High’ 
signal while ‘B Enable’ will have a ‘Low’ signal. Next, by applying a ‘Low’ signal to ‘A Direction’, 
reverses the wiring of the motors determining the robot move North-East. 

Note that, when moving a specific direction, the opposing motors that form a pair, spin in different
directions. One spins clockwise, while the other spins counter-clockwise. This was necessary, and was
achieved by wiring one motor in reverse as shown in Figure \ref{fig:reverse_wiring}.
\begin{figure}[htp]
	\centering
...
Tables \ref{tab:directions} and \ref{tab:directions_signal} represent the configurations 
...
\begin{table}[htp]
	...
	\begin{tabular}{|l|c|c|c|c|}
		\hline
		Direction & A Enable & A Direction & B Enable & B Direction \\
		\hline
		North & 1 & 0 & 1 & 0 \\
		East 	& 1 & 0 & 1 & 1 \\
		South & 1 & 1 & 1 & 1 \\
